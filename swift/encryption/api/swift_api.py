...
class Container(base_api.APIDictWrapper):
    pass


class StorageObject(base_api.APIDictWrapper):
...
class PseudoFolder(base_api.APIDictWrapper):
...
def swift_api(req):
    user = req.environ['HTTP_X_USER_NAME']
    preauthtoken = req.environ['HTTP_X_AUTH_TOKEN']

    endpoint = base_api.url_for(req, 'object-store')
    cacert = getattr(req.environ, 'openstack_ssl_cacert', None)

    LOG.debug('Swift connection created using token "%s" and url "%s"'
              % (preauthtoken, endpoint))
...
def swift_get_containers(request, marker=None):
    limit = getattr(request.environ, 'api_result_limit', 1000)
...
def swift_get_container(request, container_name, with_data=True):
    ...
    try:
        ...
        if is_public:
            swift_endpoint = base_api.url_for(request,
                                              'object-store',
                                              endpoint_type='publicURL')
            public_url = swift_endpoint + '/' + quote(container_name)
...
def swift_get_objects(request, container_name, prefix=None, marker=None,
                      limit=None):
    limit = limit or getattr(request.environ, 'api_result_limiT', 1000)
...
def swift_upload_object(request, container_name, object_name,
    ...
    size = 0
    if object_file:
        #TODO: get the file name without path
        headers['X-Object-Meta-Orig-Filename'] = object_name
        size = request.environ['CONTENT_LENGTH']
